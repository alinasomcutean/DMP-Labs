...
//const unsigned char ssdlut[] = {0b01110111, 0b01111100, 0b00111001, 0b01011110, 0b01111001};
...
void loop() {
  ...
  PORTA = (ssdlut[cpos] | outvalue); // we make an OR between the value from the LUT and the selected cathode
  cpos++; // we increment the current position

  if (cpos>=lutsize) { // if we reached the final position
    cpos = 0; // we come back at 0
    cdigit^=1; // if the previus digit was 0 we make it a 1 and vice versa
  }
  // wait 0.5 sec
  delay(500);
}
