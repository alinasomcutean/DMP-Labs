...
void setup(void)
{
  ...
  delay(1000); // perform a 1 sec delay to display this massage on the LCD
  ...
  EICRA |= (1 << ISC01); // Specify INT0 triggering behavior: falling edge of the
...
void loop()
{
  // If an interrupt was triggered/executed the LCD has to be erased and the main massage displayed
  if(buttonVariable == 1)
  {
...
ISR(INT0_vect)
{
  digitalWrite(13, !digitalRead(13)); // Change the status of pin 13
  lcd.setCursor(0,0); // Move the LCD cursor in the top-left corner
